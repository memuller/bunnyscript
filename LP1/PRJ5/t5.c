...
void main()
{
    ...
    while (n<101) // acho que a grande questão é esta aqui. (troquei o i pelo n)
    { // see? o ponto é que você precisa do i ali dentro para incrementar as coisas
    // mas o que realmente importa para achar os primos independe do que tiver feito para achar os múltiplos
    // o importante aqui é controlar o loop usando o n. o i é praticamente irrelevante fora de achar os múltiplos.
    // tenta aí.
        if (vet[i]==1)
        {
            while (i<101) // loop para marcar os múltiplos
            {
                if (i!=n)
                {
                    vet[i]=0;
            ...
            // ficamos incrementando o i para achar os múltiplos. precisamos resetar ele
            i = n;
        }
